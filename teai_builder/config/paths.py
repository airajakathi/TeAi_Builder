...
def get_workspace_path(workspace: str | None = None) -> Path:
    ...
    path = Path(workspace).expanduser() if workspace else Path.home() / ".teai_builder" / "workspace"
    return ensure_dir(path)


def is_default_workspace(workspace: str | Path | None) -> bool:
    """Return whether a workspace resolves to teai_builder's default workspace path."""
    current = Path(workspace).expanduser() if workspace is not None else Path.home() / ".teai_builder" / "workspace"
    default = Path.home() / ".teai_builder" / "workspace"
    return current.resolve(strict=False) == default.resolve(strict=False)


def get_cli_history_path() -> Path:
    """Return the shared CLI history file path."""
    return Path.home() / ".teai_builder" / "history" / "cli_history"


def get_bridge_install_dir() -> Path:
    """Return the shared WhatsApp bridge installation directory."""
    return Path.home() / ".teai_builder" / "bridge"
