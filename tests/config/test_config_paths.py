...
def test_shared_and_legacy_paths_remain_global() -> None:
    assert get_cli_history_path() == Path.home() / ".teai_builder" / "history" / "cli_history"
    assert get_bridge_install_dir() == Path.home() / ".teai_builder" / "bridge"
    assert get_legacy_sessions_dir() == Path.home() / ".teai_builder" / "sessions"


def test_workspace_path_is_explicitly_resolved() -> None:
    assert get_workspace_path() == Path.home() / ".teai_builder" / "workspace"
...
def test_is_default_workspace_distinguishes_default_and_custom_paths() -> None:
    ...
    assert is_default_workspace(Path.home() / ".teai_builder" / "workspace") is True
